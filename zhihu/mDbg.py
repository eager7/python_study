...
def DBG_Printf(flag, *x):#Blue
    if flag:
        print "\033[34;1m" + "[DBG]", "%s"*len(x)%x, "\033[0m"
def INF_Printf(flag, *x):#Yellow
    if flag:
        print "\033[33;1m" + "[INF]", "%s"*len(x)%x, "\033[0m"
def NOT_Printf(flag, *x):#Green
    if flag:
        print "\033[32;1m" + "[NOT]", "%s"*len(x)%x, "\033[0m"
def WAR_Printf(flag, *x):#Purple
    if flag:
        print "\033[35;1m" + "[WAR]", "%s"*len(x)%x, "\033[0m"
def ERR_Printf(flag, *x):#Red
    if flag:
        print "\033[31;1m" + "[ERR]", "%s"*len(x)%x, "\033[0m"
